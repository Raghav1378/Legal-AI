...
class KaggleLegalIngestor:
    ...
    into the pgvector-backed LegalVectorStore (stored in the same Postgres
    used by the backend) with full structured metadata.
    """
    ...
    def _chunk_and_ingest(
        ...
        print(f"[INGEST] '{case_name}' ({year}) → {count} chunks upserted to Postgres (pgvector).")
        return count
